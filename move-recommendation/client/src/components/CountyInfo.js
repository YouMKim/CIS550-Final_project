...
export default class CountyInfo extends React.Component {
	...
	submitCounty() {
		...
		fetch("http://localhost:8081" + window.location.pathname,
			{
				...
				let countyNameDivs = countystats.map((countyNameObj, i) =>
					<CountyName countyNameObj={countyNameObj} />
				);
				this.setState({
					countystats: countyStatsDivs,
					countypol: countyPolDivs,
					countyname: countyNameDivs
				});
			});
		fetch("http://localhost:8081/statecovid" + window.location.pathname,
			{
				method: 'GET' // The type of HTTP request.
			}).then(res => {
				console.log(res)
		...
		fetch("http://localhost:8081/countyweather" + window.location.pathname,
			{
				method: 'GET' // The type of HTTP request.
			}).then(res => {
				console.log(res)
		...
		fetch("http://localhost:8081/countyecon" + window.location.pathname,
			{
				method: 'GET' // The type of HTTP request.
			}).then(res => {
				console.log(res)
		...
		fetch("http://localhost:8081/countyzipstats" + window.location.pathname,
			{
				method: 'GET' // The type of HTTP request.
			}).then(res => {
				console.log(res)
		...
		return (
			<div className="CountyInfo">
				...
				<div className="container bestgenres-container">
					...
					<div className="h2"id="Header">
						{this.state.countyname}</div>
						<br></br>
